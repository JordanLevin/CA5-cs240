...
void Schedule::sort_semesters(){
  ...
  //while (semesters.size() >0) {
    ////pair<string, vector<string> min = sememsters[0];
    //int min = 0;
    //for (unsigned int i = 0; i < semesters.size(); i++){
       //char sem1 = semesters[i].first[0]; 
       //char sem2 = semesters[min].first[0]; 
       //int year_1 = stoi(semesters[i].first.substr(1, 4));
       //int year_2 = stoi(semesters[min].first.substr(1,4)); 
  ...
  //semesters = sorted;
  //for (unsigned int i = 0; i < semesters.size(); i++){
    //cout << "sorted: " << sorted[i].first << endl;
  //}
}
